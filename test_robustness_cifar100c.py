...
class CIFAR100C(Dataset):
    def __init__(self, root, corruption_type=None, severity=None, transform=None):
        ...
        # 加载数据
        # CIFAR-100-C数据格式: 每个corruption类型有一个.npy文件
...
def load_cifar100c_data(root, batch_size=256, num_workers=2, corruption_type=None, severity=None):
    """
    加载CIFAR-100-C数据集
    ...
    dataset = CIFAR100C(root=root, corruption_type=corruption_type, severity=severity, transform=test_transform)
...
def test_robustness(args):
    ...
    for corr_type in corruption_types:
        ...
        for severity in range(1, 6):
            try:
                # 加载该失真类型和强度的数据
                test_loader = load_cifar100c_data(
                    root=args.data_root,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CIFAR-100-C鲁棒性测试')
    ...
    parser.add_argument("--checkpoint_path", 
                       default='/home/weking/sharpness-aware-minimization/checkpoint/complexity/best_CIFAR100_ResNet18_batch_128_D_FriendlySAM_lr_0.1_wd_0.0005_mom_0.9_rho_4.0_epochs_200_seed_0_noise_0.0.pth',
                       type=str, help="模型checkpoint路径") 
						
    parser.add_argument("--data_root", 
                       default='/data/weking/CIFAR-100-C',
                       type=str, help="CIFAR-100-C数据集根目录")
    ...
    parser.add_argument("--output_dir", default='/home/weking/sharpness-aware-minimization/checkpoint/robustness_results_cifar100c/', type=str, help="结果保存目录")
